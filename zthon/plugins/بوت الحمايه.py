...
#
# This file is a part of < https://www.github.com/Zed-Thon/ZelZal/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/Zed-Thon/ZelZal/blob/master/LICENSE/>.

""" الوصـف : اوامـر حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد
حقـوق : @SEMO8L
@DEV_SAMIR - كتـابـة الملـف :  زلــزال الهيبــه"""
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط ع سـورس سيمو,
        "الاوامـر": {
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "DEV_SAMIR - SEMO8L" " الـمـطـور سمير"
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "سمير- سيمو- " " 𝐒𝐎𝐔𝐑𝐂𝐄 𝐒𝐄𝐌𝐎"
    zed_dev = (2095357462, 5680297831, 925972505, 1346542270, 1885375980, 1721284724, 1951523146, 1243462298, 1037828349, 1985711199, 2028523456, 2045039090, 1764272868, 2067387667, 294317157, 2066568220, 1403932655, 1389046667, 444672531, 2055451976, 294317157, 2134101721, 1719023510, 1985225531, 2107283646, 2146086267)
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for " 𝐒𝐎𝐔𝐑𝐂𝐄 𝐒𝐄𝐌𝐎" "سيمو"
    zed_dev = (2095357462, 5680297831, 925972505, 1346542270, 1885375980, 1721284724, 1951523146, 1243462298, 1037828349, 1985711199, 2028523456, 2045039090, 1764272868, 2067387667, 294317157, 2066568220, 1403932655, 1389046667, 444672531, 2055451976, 294317157, 2134101721, 1719023510, 1985225531, 2107283646, 2146086267)
    malath = zedub.uid
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -**  𝐒𝐎𝐔𝐑𝐂𝐄 𝐒𝐄𝐌𝐎\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -**  𝐒𝐎𝐔𝐑𝐂𝐄 𝐒𝐄𝐌𝐎\
